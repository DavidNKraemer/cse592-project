...
optimizer = SdLBFGS(lambda x: tanh_obj(x, order=1,
                                       data=data, minibatch_size=1,regularizer=1e-4),
...
print('Objective function', tanh_obj(sdlbfs_x[-1], order=0,
                                     data=data, minibatch_size=len(labels), regularizer=1e-4))
...
obj_f = lambda x, order: tanh_obj(x, order=order, data=data, minibatch_size=10, regularizer=1e-4)
...
sgd_pred = [np.sign(np.dot(sgd_xs[sgd_minind].T, feature.T)).item(0) for feature in features]
print(sgd_corr)
...
print('Objective function', tanh_obj(lbfgs_x[-1], order=0,
                                     data=data, minibatch_size=len(labels), regularizer=1e-4))
...
plt.suptitle(r'SVM loss function $f(x) = 1-\tanh(x) + \lambda|x|^{2}$')
sns.despine()

fig.savefig('../plots/svm_tan_regularized_loss.eps', bbox_inches='tight')
#%%
